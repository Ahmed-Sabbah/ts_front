...
export default {
  ...
  methods: {
    ...
    async chooseUploadedBy(name, id) {
      ...
      $(".uploadedBy").removeClass("open");
      await axios
        .get("/media/filter?uploader_id=" + id + "&share_id=" + this.shareID)
        .then((res) => {
  ...
  watch: {
    mediaName: function (v) {
      if (v == "") {
        ...
        axios
          .get(
            "/media/filter/search?searchable_id=" +
              this.id +
              "&searchable_type=" +
              this.type +
              "&s_key=" +
  ...
  async mounted() {
    ...
    await axios
      .get("/get_family_tags?share_id=" + this.$route.params.shareID)
      .then((res) => {
        if (res.data.status.success) {
    ...
    await axios
      .get("/get_family_contributors?share_id=" + this.$route.params.shareID)
      .then((res) => {
        if (res.data.status.success) {
          this.usersList = res.data.contributors;
        }
      });
