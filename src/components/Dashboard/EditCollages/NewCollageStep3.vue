...
export default {
  ...
  methods: {
    ...
    async saveCollage() {
      if (this.i_num_photo == this.s_media_ids.length) {
        ...
        await html2canvas(document.querySelector(orientationLayout), {
          ...
          // let images = $(`${orientationLayout} img`);
        ...
        collage.set("pk_i_id", this.pk_i_id);
    ...
    async saveDraft() {
      ...
      let images = document.querySelectorAll(`${orientationLayout} img`);
      ...
      draftCollage.set("pk_i_id", this.pk_i_id);
  ...
  async mounted() {
    ...
    for (const [key, value] of Object.entries(this.layoutMedia)) {
      let img = new Image();
      const targetMedia = this.mediaList.filter((key) => key.pk_i_id == value);
      img.src = targetMedia[0].s_file;
      img.id = "drag" + value;
      img.setAttribute("data-id", value);
      $(".create-collage#step3 " + orientationLayout + " #" + key).append(img);

      this.orderKeysArr.push(key);
      this.orderValuesArr.push(value);
    }
  },
};
</script>
