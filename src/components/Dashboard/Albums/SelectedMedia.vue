<template>
  <div class="select-media-preview">
    ...
    <p v-if="selectState">
      Albums ( {{ selectedMediaArr.length }} media selected )
    </p>
    ...
    <button
      ...
      id="loadingBtn"
      v-if="
        selectState && coverState && albumNameVal != '' && submitType == 'new'
      "
      @click="saveAlbum"
    >
      Save Album <img src="../../../assets/images/loading.gif" alt="loading" />
    </button>
    <button
      class="save-btn"
      id="loadingBtn"
      v-if="
        selectState && coverState && albumNameVal != '' && submitType == 'edit'
      "
      @click="updateAlbum(albumID)"
    >
      Update Album <img src="../../../assets/images/loading.gif" alt="loading" />
    </button>
    <button
    ...
    <button
      ...
      v-if="
        !selectState ||
        !coverState ||
        (albumNameVal == '' && submitType == 'edit')
      "
    >
      Update Album
    </button>
    <div class="empty-selected-media" v-if="!selectState">
      <h3>Select media<br />for your album</h3>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    saveAlbum() {
      this.albumErrors = [];
      this.$emit("updateErrors", this.albumErrors);

      if (this.albumNameVal == "") {
        this.albumErrors.push("Album name is required.");
        this.$emit("updateErrors", this.albumErrors);
      } else {
        $(".create-album .select-media-preview #loadingBtn").prop("disabled", true);
        axios
          .post("/albums/add_edit", {
            s_name: this.albumNameVal,
          ...
          .then(
            (res) => {
              if (res.data.status.success) {
                //show success model
                $(".create-album .select-media-preview #loadingBtn").prop("disabled", false);
                $(".create-album .model-container#albumSuccessModel").css(
                  "display",
                  "flex"
                );
              }
...
<style scoped>
@import url("../../../assets/css/select-media-preview.css");
</style>
