...
export default {
  ...
  methods: {
    ...
    selectTag(tag) {
      ...
      if (check > -1) {
        ...
        if (this.selectedTags.length == 0) {
          ...
          axios.get("/archive?tags_id=" + this.selectedTags).then((res) => {
            if (res.data.status.success) {
              this.media = res.data.media;
              this.$emit("updateMediaList", this.media);
            }
    ...
    filterMediaByTimeline(e) {
      ...
      axios
        .get(
          "/archive?i_year_from=" + this.period1 + "&i_year_to=" + this.period2
        )
        .then((res) => {
          if (res.data.status.success) {
            this.media = res.data.media;
  ...
  computed: {
    uploadedByListFilter: function () {
      ...
      return this.usersList.filter((el) => el.match(filtering));
    },
  ...
  watch: {
    mediaName: function (v) {
      if (v == "") {
        ...
        axios.get("/archive/search?s_key=" + v).then((res) => {
          if (res.data.status.success) {
            this.media = res.data.media;
            this.$emit("updateMediaList", this.media);
          }
