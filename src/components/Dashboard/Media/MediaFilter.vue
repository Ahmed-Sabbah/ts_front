          ...
          <div class="uploadedBy-dropdown">
            ...
            <div class="select-value">
              <span
                ...
                @click="
                  chooseUploadedBy(contributor.s_fullname, contributor.pk_i_id)
                "
...
export default {
  ...
  methods: {
    ...
    selectTag(tag) {
      ...
      if (check > -1) {
        ...
        if (this.selectedTags.length == 0) {
          ...
          axios.get("/media?tags_id=" + this.selectedTags).then((res) => {
            if (res.data.status.success) {
              this.media = res.data.media;
              this.$emit("updateMediaList", this.media);
            }
    ...
    filterMediaByTimeline(e) {
      ...
      axios
        .get(
          "/media?i_year_from=" + this.period1 + "&i_year_to=" + this.period2
        )
        .then((res) => {
          if (res.data.status.success) {
            this.media = res.data.media;
  ...
  watch: {
    mediaName: function (v) {
      if (v == "") {
        ...
        axios.get("/media/search?s_key=" + v).then((res) => {
          if (res.data.status.success) {
            this.media = res.data.media;
            this.$emit("updateMediaList", this.media);
          }
  ...
  async mounted() {
    ...
    //Uploaded By
    await axios.get("/users?b_uploader=true").then((res) => {
      if (res.data.status.success) {
    ...
    for (let index = 0; index < decadesArr.length; index++) {
      ...
      $(".media-filter .timeline .timeline-dropdown .periods").append(
        decadeItem
      );
    }
  },
