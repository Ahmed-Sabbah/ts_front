<template>
  <div class="preview-media">
        ...
        <div class="information" v-if="previewState">
          ...
          <div class="data">
            ...
            <div>
              ...
              <div
                :class="
                  item.selected == true && item.b_unpublished == false
                    ? 'comment selected'
                    : item.b_unpublished == true
                    ? 'comment unpublished'
                    : 'comment'
                "
    ...
    <div class="model-container" id="archiveMediaConfirm">
      <div class="model-box">
        <h3 class="model-title">Archive Media</h3>
        <p>Are you sure you want to archive this media ?</p>
        <div class="btn-group">
...
export default {
  ...
  methods: {
    ...
    addComment(mediaID) {
      ...
      if (comment == "") {
        this.$toast.success("Comment Required!");
      } else {
        axios
          .post("/media/action", {
  ...
  async mounted() {
    await axios.get("/media?e_type=PHOTO").then((res) => {
      if (res.data.status.success) {
