<template>
  ...
  <div class="preview-memorial step1">
    <div class="selected-memorial-cover">
      <img :src="memorialCoverObj.s_file" alt="memorial-cover" v-if="memorialCoverObj" />
      <img :src="memorialCover" alt="memorial-cover" v-else />
    </div>
    <div class="btn-group">
      <button class="back-btn" @click="cancelMemorial">Back</button>
      <button class="next-btn" @click="showStep2">Next</button>
    </div>
  </div>
</template>
...
<script>
import $ from "jquery";
import MediaFilter from "../Memorials/MediaFilter.vue";
import MediaBox from "../Memorials/MediaBox.vue";
import axios from "axios";

export default {
  name: "NewMemorial",
  components: { MediaBox, MediaFilter },
  props: ["s_cover", "memorialCover"],
  data() {
    return {
      mediaList: [],
      emptyState: false,
