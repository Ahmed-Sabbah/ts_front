...
export default {
  ...
  methods: {
    ...
    unselectTags() {
      ...
      this.selectedTags = [];
      axios.get("/media?e_type=PHOTO").then((res) => {
        if (res.data.status.success) {
          this.media = res.data.media;
          this.$emit("updateMediaList", this.media);
        }
    ...
    selectTag(tag) {
      ...
      if (check > -1) {
        ...
        if (this.selectedTags.length == 0) {
          ...
          axios
            .get("/media?e_type=PHOTO&tags_id=" + this.selectedTags)
            .then((res) => {
              if (res.data.status.success) {
                this.media = res.data.media;
                this.$emit("updateMediaList", this.media);
    ...
    filterMediaByTimeline(e) {
      ...
      axios
        .get(
          "/media?e_type=PHOTO&i_year_from=" +
            this.period1 +
            "&i_year_to=" +
            this.period2
        )
  ...
  watch: {
    mediaName: function (v) {
      if (v == "") {
        ...
        axios.get("/media/search?e_type=PHOTO&s_key=" + v).then((res) => {
          if (res.data.status.success) {
            this.media = res.data.media;
            this.$emit("updateMediaList", this.media);
          }
