  ...
  <div class="create-journey">
    <div class="create-journey-box">
      ...
      <div class="journey-steps">
        <p>Step {{ stepNumber }} of 2: <span>Journey Media</span></p>
        <div class="range">
...
export default {
  ...
  async mounted() {
    ...
    await axios.get("/journeys?id=" + this.journeyID).then((res) => {
      if (res.data.status.success) {
        for (let index = 0; index < res.data.media[0].media.length; index++) {
          this.selectedMedia.push(res.data.media[0].media[index]);
          this.submitedMediaId.push(res.data.media[0].media[index].pk_i_id);
        }
        this.coverObj = res.data.media[0].s_cover;
        this.coverID = res.data.media[0].s_cover.pk_i_id;
        this.selectState = true;
        this.journyName = res.data.media[0].s_name;
        this.journyDescription = res.data.media[0].s_description;
